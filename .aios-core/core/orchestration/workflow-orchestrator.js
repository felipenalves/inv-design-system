...
class WorkflowOrchestrator {
  ...
  constructor(workflowPath, options = {}) {
    ...
    this.options = {
      ...
      confidenceThreshold: this._resolveConfidenceThreshold(
  ...
  _getNextPhaseHandoffTarget(currentPhaseNum) {
    const sequence = Array.isArray(this.workflow?.sequence)
      ? this.workflow.sequence
      : []
    const currentIndex = sequence.findIndex(
      (p) => p && p.phase === currentPhaseNum,
    )
    if (currentIndex < 0) {
  ...
  _generateExecutionSummary() {
    ...
    const deliveryConfidence =
      this.contextManager?.getDeliveryConfidence?.() || null
    const summary = {
