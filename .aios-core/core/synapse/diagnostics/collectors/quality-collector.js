...
const UAP_RUBRIC = [
  ...
  },
  {
    ...
    weight: 12,
    criticality: 'MEDIUM',
    impact: 'Project status context',
  },
  {
    name: 'gitConfig',
    weight: 8,
    criticality: 'LOW',
    impact: 'Branch name context',
  },
  {
    name: 'permissionMode',
    weight: 5,
    criticality: 'LOW',
    impact: 'Permission badge visual',
  },
  {
    name: 'synapseSession',
    weight: 5,
    criticality: 'LOW',
    impact: 'Bridge write for SYNAPSE',
  },
]
...
const HOOK_RUBRIC = [
  {
    ...
    weight: 25,
    criticality: 'CRITICAL',
    impact: 'Framework principles',
  },
  {
    name: 'agent',
    weight: 25,
    criticality: 'CRITICAL',
    impact: 'Agent-specific instructions',
  },
  {
    name: 'global',
    weight: 20,
    criticality: 'CRITICAL',
    impact: 'Project-wide rules',
  },
  {
    name: 'workflow',
    weight: 10,
    criticality: 'MEDIUM',
    impact: 'Workflow context',
  },
  {
    name: 'task',
    weight: 10,
    criticality: 'MEDIUM',
    impact: 'Task instructions',
  },
  { name: 'squad', weight: 5, criticality: 'LOW', impact: 'Squad rules' },
  {
    ...
    weight: 3,
    criticality: 'LOW',
    impact: 'Keyword triggers',
  },
  {
    name: 'star-command',
    weight: 2,
    criticality: 'LOW',
    impact: 'Command-specific rules',
  },
]
...
function collectQualityMetrics(projectRoot) {
  ...
  const uapAge =
    uapData && uapData.timestamp
      ? now - new Date(uapData.timestamp).getTime()
      : 0
  const hookAge =
    hookData && hookData.timestamp
      ? now - new Date(hookData.timestamp).getTime()
      : 0
  ...
  let uapNormalized =
    uap.maxPossible > 0 ? (uap.score / uap.maxPossible) * 100 : 0
  let hookNormalized =
    hook.maxPossible > 0 ? (hook.score / hook.maxPossible) * 100 : 0
...
function _scoreHook(data) {
  ...
  const activeLayers =
    BRACKET_ACTIVE_LAYERS[bracket] || BRACKET_ACTIVE_LAYERS.MODERATE
