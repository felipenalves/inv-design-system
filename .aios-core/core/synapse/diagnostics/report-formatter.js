...
function formatReport(data) {
  ...
  if (agentId) {
    const quality =
      data.session?.raw?.bridgeData?.activation_quality || 'unknown'
  ...
  if (data.session && data.session.fields) {
    ...
    for (const field of data.session.fields) {
      lines.push(
        `| ${field.field} | ${field.expected} | ${field.actual} | ${field.status} |`,
      )
    }
  } else {
  ...
  lines.push(
    `## 4. Pipeline Simulation (${data.pipeline?.bracket || 'UNKNOWN'} bracket)`,
  )
  ...
  if (gaps.length === 0) {
    ...
    for (let i = 0; i < gaps.length; i++) {
      lines.push(
        `| ${i + 1} | ${gaps[i].severity} | ${gaps[i].gap} | ${gaps[i].recommendation} |`,
      )
    }
  }
  lines.push('')
...
function _extractAgentId(data) {
  ...
  if (data.session?.raw?.session?.active_agent?.id)
    return data.session.raw.session.active_agent.id
  return null
}
  ...
  for (const g of relevanceGaps) {
    if (g.importance === 'critical') {
      gaps.push({
        ...
        recommendation:
          'Ensure critical context component is available for active agent',
      })
    }
  }
...
function _formatTimingSection(lines, timing) {
  ...
  if (timing.uap && timing.uap.available) {
    ...
    for (const loader of timing.uap.loaders) {
      lines.push(
        `| ${loader.name} | ${loader.duration}ms | ${loader.status} | ${loader.tier} |`,
      )
    }
  ...
  if (timing.hook && timing.hook.available) {
    ...
    for (const layer of timing.hook.layers) {
      lines.push(
        `| ${layer.name} | ${layer.duration}ms | ${layer.status} | ${layer.rules} |`,
      )
    }
    const totalRules = timing.hook.layers.reduce(
      (sum, l) => sum + (l.rules || 0),
      0,
    )
    lines.push(
      `| **Total** | **${timing.hook.totalDuration}ms** | | **${totalRules}** |`,
    )
  } else {
  ...
  // Combined
  lines.push(
    `**Combined pipeline time:** ${timing.combined ? timing.combined.totalMs : 0}ms`,
  )
  lines.push('')
}
...
function _formatQualitySection(lines, quality) {
  ...
  lines.push(
    `**Overall: ${overall.score}/100 (${overall.grade} — ${overall.label})**`,
  )
  ...
  lines.push(
    `UAP: ${uapScore}/100${uapStale} | Hook: ${hookScore}/100${hookStale}`,
  )
  lines.push('')
...
function _formatOutputSection(lines, outputAnalysis) {
  ...
  if (outputAnalysis.hookAnalysis && outputAnalysis.hookAnalysis.length > 0) {
    ...
    for (const a of outputAnalysis.hookAnalysis) {
      lines.push(
        `| ${a.name} | ${a.status} | ${a.rules} | ${a.quality} | ${a.detail} |`,
      )
    }
    lines.push('')
  }
...
function _formatRelevanceSection(lines, relevance) {
  ...
  lines.push(
    `**Agent:** @${relevance.agentId} | **Relevance Score:** ${relevance.score}/100`,
  )
  lines.push('')
  ...
  if (relevance.matrix && relevance.matrix.length > 0) {
    ...
    for (const item of relevance.matrix) {
      ...
      lines.push(
        `| ${item.component} | ${item.importance} | ${item.status} | ${gapFlag} |`,
      )
    }
    lines.push('')
  }
  ...
  if (relevance.gaps && relevance.gaps.length > 0) {
    ...
    for (const gap of relevance.gaps) {
      lines.push(
        `- **${gap.component}** (${gap.importance}): missing or failed`,
      )
    }
    lines.push('')
  }
