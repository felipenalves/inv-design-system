...
function isParsableAgent(agent) {
  return (
    !agent.error ||
    agent.error === 'YAML parse failed, using fallback extraction'
  )
}
...
function validateSkillContent(content, expected) {
  ...
  const requiredChecks = [
    ...
    {
      ok: content.includes(
        `.aios-core/development/agents/${expected.filename}`,
      ),
...
function validateCodexSkills(options = {}) {
  ...
  for (const item of expected) {
    ...
    if (!fs.existsSync(skillPath)) {
      ...
      errors.push(
        `Missing skill file: ${path.relative(resolved.projectRoot, skillPath)}`,
      )
      continue
    }
    ...
    try {
      ...
      errors.push(
        `${item.skillId}: unable to read skill file (${error.message})`,
      )
      continue
    }
