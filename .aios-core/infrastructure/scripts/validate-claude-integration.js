...
function validateClaudeIntegration(options = {}) {
  ...
  const rulesFile =
    options.rulesFile || path.join(projectRoot, '.claude', 'CLAUDE.md')
  const agentsDir =
    options.agentsDir ||
    path.join(projectRoot, '.claude', 'commands', 'AIOS', 'agents')
  const hooksDir =
    options.hooksDir || path.join(projectRoot, '.claude', 'hooks')
  ...
  if (!fs.existsSync(agentsDir)) {
    errors.push(
      `Missing Claude agents dir: ${path.relative(projectRoot, agentsDir)}`,
    )
  }
  if (!fs.existsSync(rulesFile)) {
    warnings.push(
      `Claude rules file not found yet: ${path.relative(projectRoot, rulesFile)}`,
    )
  }
  if (!fs.existsSync(hooksDir)) {
    warnings.push(
      `Claude hooks dir not found yet: ${path.relative(projectRoot, hooksDir)}`,
    )
  }
  ...
  if (sourceCount > 0 && claudeCount !== sourceCount) {
    warnings.push(
      `Claude agent count differs from source (${claudeCount}/${sourceCount})`,
    )
  }
