...
function validateGeminiIntegration(options = {}) {
  ...
  const resolved = {
    ...
    projectRoot,
    rulesFile:
      options.rulesFile || path.join(projectRoot, '.gemini', 'rules.md'),
    agentsDir:
      options.agentsDir ||
      path.join(projectRoot, '.gemini', 'rules', 'AIOS', 'agents'),
    commandsDir:
      options.commandsDir || path.join(projectRoot, '.gemini', 'commands'),
    extensionDir:
      options.extensionDir ||
      path.join(projectRoot, 'packages', 'gemini-aios-extension'),
    sourceAgentsDir:
  ...
  if (sourceCount > 0 && geminiCount !== sourceCount) {
    warnings.push(
      `Gemini agent count differs from source (${geminiCount}/${sourceCount})`,
    )
  }
  ...
  for (const rel of requiredExtensionFiles) {
    ...
    if (!fs.existsSync(abs)) {
      errors.push(
        `Missing Gemini extension file: ${path.relative(resolved.projectRoot, abs)}`,
      )
    }
  }
