...
function parseArgs(argv = process.argv.slice(2)) {
  const args = new Set(
    argv.filter(
      (arg) => !arg.startsWith('--contract=') && !arg.startsWith('--diff='),
    ),
  )
...
function runSyncValidate(ide, projectRoot) {
  ...
  const result = spawnSync(
    'node',
    [script, 'validate', '--ide', ide, '--strict'],
    {
      cwd: projectRoot,
      encoding: 'utf8',
    },
...
function normalizeResult(input) {
  if (!input || typeof input !== 'object') {
    return {
      ok: false,
      errors: ['Validator returned invalid result'],
      warnings: [],
    }
  }
  return {
...
function validateCompatibilityContract(contract, resultById, options = {}) {
  ...
  if (!docsPath || !fs.existsSync(docsPath)) {
    violations.push(
      `Compatibility matrix document not found: ${docsPath || 'undefined'}`,
    )
    return violations
  }
  ...
  for (const ide of matrix) {
    ...
    const requiredChecks = Array.isArray(ide.required_checks)
      ? ide.required_checks
      : []
    ...
    for (const checkId of requiredChecks) {
      ...
      if (!checkResult) {
        violations.push(
          `Contract requires unknown check "${checkId}" for IDE "${ideName}"`,
        )
        continue
      }
      if (!checkResult.ok) {
        violations.push(
          `Contract violation for "${ideName}": required check "${checkId}" failed`,
        )
      }
    }
  }
  ...
  for (const checkId of globalRequiredChecks) {
    ...
    if (!checkResult.ok) {
      violations.push(
        `Contract violation: global required check "${checkId}" failed`,
      )
    }
  }
...
function diffCompatibilityContracts(currentContract, previousContract) {
  ...
  const currentGlobalChecks = sortUnique(
    currentContract.global_required_checks || [],
  )
  const previousGlobalChecks = sortUnique(
    previousContract.global_required_checks || [],
  )
  ...
  const ideKeys = sortUnique([
    ...Object.keys(currentByIde),
    ...Object.keys(previousByIde),
  ])
  ...
  for (const ide of ideKeys) {
    ...
    const checksAdded = currentChecks.filter(
      (item) => !previousChecks.includes(item),
    )
    const checksRemoved = previousChecks.filter(
      (item) => !currentChecks.includes(item),
    )
...
function runParityValidation(options = {}, deps = {}) {
  ...
  const runClaudeIntegration =
    deps.validateClaudeIntegration || validateClaudeIntegration
  const runCodexIntegration =
    deps.validateCodexIntegration || validateCodexIntegration
  const runGeminiIntegration =
    deps.validateGeminiIntegration || validateGeminiIntegration
  ...
  const loadContract =
    deps.loadCompatibilityContract || loadCompatibilityContract
  const contract = loadContract(resolvedContractPath)
  const resolvedDiffPath = options.diffPath
    ? path.resolve(projectRoot, options.diffPath)
    : null
  const previousContract = resolvedDiffPath
    ? loadContract(resolvedDiffPath)
    : null
  ...
  const contractViolations = validateCompatibilityContract(
    contract,
    resultById,
    {
      docsPath,
...
function formatHumanReport(result) {
  ...
  if (result.contract && result.contract.release) {
    lines.push(
      `Compatibility Contract: ${result.contract.release} (${result.contract.path})`,
    )
    lines.push('')
  }
  if (result.contractDiff) {
    ...
    if (!result.contractDiff.has_changes) {
      ...
      const globalRemoved =
        result.contractDiff.global_required_checks.removed || []
  ...
  if (
    Array.isArray(result.contractViolations) &&
  ...
  lines.push('')
  lines.push(
    result.ok ? '✅ Parity validation passed' : '❌ Parity validation failed',
  )
  return lines.join('\n')
}
