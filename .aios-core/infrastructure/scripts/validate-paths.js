  ...
  lines.forEach((line, index) => {
    for (const pattern of FORBIDDEN_ABSOLUTE_PATTERNS) {
      ...
      for (const match of matches) {
        errors.push(
          `${filePath}:${index + 1} forbidden absolute path "${match}"`,
        )
      }
    }
  })
...
function validateSkillPathConventions(content, filePath) {
  ...
  if (!content.includes('.aios-core/development/agents/')) {
    errors.push(
      `${filePath} missing canonical source path ".aios-core/development/agents/"`,
    )
  }
  if (
...
function validatePaths(options = {}) {
  ...
  for (const file of filesToCheck) {
    if (!fs.existsSync(file)) {
      errors.push(
        `Missing required file: ${path.relative(resolved.projectRoot, file)}`,
      )
      continue
    }
    ...
    errors.push(
      ...collectAbsolutePathViolations(
        content,
        path.relative(resolved.projectRoot, file),
      ),
    )
